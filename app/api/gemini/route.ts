...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      "Gemini API key found, length:",
      process.env.NEXT_PUBLIC_GEMINI_API_KEY.length
    );
    console.log(
      "API key starts with:",
    ...
    // Try multiple AI services: OpenAI first, then Gemini, then local fallback
    let useOpenAI = false;
    let useGemini = false;
    let model = null;
    ...
    if (process.env.OPENAI_API_KEY) {
      try {
        console.log("Attempting to use OpenAI GPT-4...");
        useOpenAI = true;
    ...
    if (!useOpenAI) {
      try {
        ...
        console.log(
          "Gemini API not available, using local generation:",
          error.message
        );
        useGemini = false;
      }
...
`;
    ...
    if (useOpenAI) {
      try {
        ...
        console.log("Using plain text response from OpenAI");
        return NextResponse.json({
          content: text,
        });
    ...
    if (!useOpenAI && useGemini && model) {
      try {
        ...
        console.error(
          "Gemini API call failed, falling back to local generation:",
          modelError.message
        );
        useGemini = false;
      }
    ...
    if (!useOpenAI && !useGemini) {
      ...
      const localResponse = generateLocalPresentation(
        prompt,
        isEdit,
        currentPPT
      );
